...
<?php if ( $vid != '' and !$main_error and $form_status != '' ) { ?>
	...
	WHERE vendor_contacts.vendor_id = " . $_REQUEST['vid'] . " AND vendor_contacts.active = 1";
	$result_list = mysql_query($sql, $link);
	if ( mysql_num_rows($result_list) > 0 ) {
		$bg = 0; ?>
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="3">
...
<?php if ( $vid != '' and $_REQUEST['update'] != 1 ) { ?>
	...
	<?php if ( $_REQUEST['action'] == 'search' ) { ?>
		...
		WHERE vendor_id = " . $vid . $vendor_name_clause . $VendorProductCode_clause . $ProductNumberInternal_clause . $Designation_clause . " ORDER BY Designation";
