...
if ( isset($_POST['subform']) ) { // NUMBERS FORM
	...
	if ( !$error_found ) {
		...
		if ( $pid != "" ) {
			$sql = "UPDATE customer_address_phones " .
			...
			$sql = "INSERT INTO customer_address_phones (address_id, number_description, number, type) VALUES (" . $aid . ",'" . $number_description . "', '" . $number . "', " . $type . ")";
			mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
		}
		$_SESSION['subnote'] = "Information successfully saved<BR>";
		header("location: customers_customers.add_edit.php?cid=" . $cid . "&aid=" . $aid);
		exit;
	} else {
		$sub_errors = true;
	}
...
if ( !empty($_POST) and !$sub_errors ) { // MAIN FORM
	...
	if ( !$error_found ) {
		...
		if ( $aid != "" ) {
			...
			$sql = "INSERT INTO customer_addresses (customer_id, address1, address2, city, state, zip, country, notes) VALUES (" . $cid . ", '" . $address1 . "', '" . $address2 . "', '" . $city . "', '" . $state . "', '" . $zip . "', '" . $country . "', '" . $notes . "')";
			mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
		}

		$_SESSION['note'] = "Address information successfully saved<BR>";
		header("location: customers_customers.edit.php?cid=" . $cid);
		exit();
	...
	if ( $aid != '' ) {
		$sql = "SELECT * FROM customer_addresses WHERE address_id = " . $aid;
...
if ( $_GET['action'] == "inact" ) {
	$sql = "DELETE from customer_address_phones WHERE phone_id = " . $_GET['pid'];
	mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	header("location: customers_customers.add_edit.php?cid=" . $_GET['cid'] . "&aid=" . $aid);
	exit();
}
...
<FORM METHOD="post" ACTION="customers_customers.add_edit.php">
...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
	...
	<TR>
		<TD></TD>
		<TD><INPUT TYPE='submit' VALUE="Save"> <INPUT TYPE='button' VALUE="Cancel" onClick="window.location='customers_customers.edit.php?cid=<?php echo $cid;?>'"></TD>
	</TR></FORM>
</TABLE>
...
<?php if ( isset($aid) and !$main_error ) { ?>
	...
	if ( isset($pid) ) {
		$sql = "SELECT * FROM customer_address_phones WHERE phone_id = " . $pid;
	...
	<?php
	$sql = "SELECT * FROM customer_address_phones LEFT JOIN phone_types ON customer_address_phones.type = phone_types.type_id WHERE address_id = " . $aid . " ORDER BY type_id";
	$result = mysql_query($sql, $link);
	if ( mysql_num_rows($result) > 0 ) {
		$bg = 0; ?>
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			if ( $bg == 1 ) {
				...
				<TR BGCOLOR="<?php echo($bgcolor);?>" VALIGN=TOP>
					<TD><A HREF="customers_customers.add_edit.php?cid=<?php echo $cid;?>&pid=<?php echo $row['phone_id'];?>&aid=<?php echo $aid;?>"><IMG SRC="images/pencil.gif" WIDTH="16" HEIGHT="16" BORDER=0></A></TD>
					<TD><A HREF="JavaScript:inactivate(<?php echo($row['phone_id'] . "," . $cid . "," . $aid);?>)"><IMG SRC="images/delete.gif" WIDTH="16" HEIGHT="16" BORDER="0"></A></TD>
...
function inactivate(pid, cid, aid) {
	if ( confirm('Are you sure you want to delete this number?') ) {
		document.location.href = "customers_customers.add_edit.php?action=inact&pid=" + pid + "&cid=" + cid + "&aid=" + aid
	}
}
