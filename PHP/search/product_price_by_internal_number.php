...
$VendorID=(isset($_REQUEST['VendorID'])) ? escape_data($_REQUEST['VendorID']) : "";
$vendid_clause=($VendorID == "" )? "" : " AND vendors.vendor_id=$VendorID";
$sql = "SELECT DISTINCT pm.ProductNumberInternal as pni,pm.Designation, epnr.ProductNumberExternal, pp.*, name
	...
	WHERE pm.ProductNumberInternal LIKE '%$q%' AND pp.is_deleted = 0 and vendors.name NOT LIKE '%Abelei%' $vendid_clause";
...
if (0 < $result_count)
{
	while ( $row = mysql_fetch_array($result, MYSQL_ASSOC) ) {
		$dsp_msg=$row["ProductNumberInternal"];
