...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1">
	<TR>
		...
		<TD>&nbsp;&nbsp;&nbsp;</TD>
		<TD><B>Internal#:</B></TD>
		<TD><?php echo $row_header['ProductNumberInternal'];?></TD>
		<TD>&nbsp;&nbsp;&nbsp;</TD>
		<?php if ( $locked == 1 ) { ?>
...
<?php if ( basename($_SERVER['PHP_SELF']) != 'customers_quotes.email.php' ) { ?>
	...
	if ( mysql_num_rows($result_letters) > 0 ) {
		...
		echo "<TD><B>Date letter sent</B></TD>";
		echo "</TR>";
		while ( $row_letters = mysql_fetch_array($result_letters) ) {
			echo "<TR VALIGN=TOP>";
			echo "<TD><NOBR><A HREF='reports/pricing_quote_letter.php?reprint=1&psn=" . $_REQUEST['psn'] ."&address_id=" . $row_letters['address_id'] . "&contact_name=" . $row_letters['contact_name'] . "' TARGET='_blank'>" . $row_letters['contact_name'] . "</A>&nbsp;&nbsp;</NOBR></TD>";
			//echo "<TD>" . $row_letters['address1'] . " " . $row_letters['address2'] . " " . $row_letters['city'] . ", " . $row_letters['state'] . " " . $row_letters['zip'] . "</TD>";
			echo "<TD>" . date("m/d/Y", strtotime($row_letters['datetime_sent'])) . "</TD>";
			echo "</TR>";
		}
		echo "</TABLE>";
	}
