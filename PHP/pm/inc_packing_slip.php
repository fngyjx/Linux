...
$sql = "SELECT first_name, last_name, company, address1, address2, city, state, zip, country, sample_size, sample_size_other, project_id, date_created, company_id, salesperson, shipped_date, shipper, shipper_other, shipping FROM projects LEFT JOIN clients USING(client_id) LEFT JOIN companies USING(company_id) WHERE project_id = " . $_GET['pid'];
...
if ( $c == 0 ) {
	...
	while ( $row_flavor = mysql_fetch_array($result_flavor) ) {
		...
		$message .= "</TD>";
		$message .= "<TD ALIGN=CENTER WIDTH=100><B>";
		//if ( $row_flavor['suggested_level'] < 3 ) {
		//	$message .= $suggested_level_array[$row_flavor['suggested_level']-1];
		//} else {
