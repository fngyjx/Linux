...
if ( isset($_POST['add_prod']) ) {
	...
	if ( !$error_found ) {
		...
		$sql = "INSERT INTO customerorderdetail (CustomerOrderNumber, ProductNumberInternal, CustomerOrderSeqNumber, Quantity, PackSize, UnitOfMeasure, TotalQuantityOrdered, CustomerCodeNumber, BilledDate, OrderPackIn) VALUES ('" . $order_num . "', '" . $ProductNumberInternal . "', '" . $CustomerOrderSeqNumber . "', '" . $Quantity . "', '" . $PackSize . "', '" . $UnitOfMeasure . "', '" . $TotalQuantityOrdered . "', '" . $CustomerCodeNumber . "', " . $NewBilledDate . ",'".$Packaged_In."')";
		mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
...
if ( isset($_POST['edit_prod']) ) {
	...
	$CustomerCodeNumber = $_POST['CustomerCodeNumber'];
	$Packaged_In = escape_data($_POST['Packaged_In']);
...
//Customer OrderMaster info update
	...
	if ( $billing_id != "" ) {
		$sql = "SELECT * FROM customer_addresses WHERE address_id = $billing_id";
		// echo "<br /> $sql <br />";
		$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
		if ( mysql_num_rows($result) > 0 ) 
			$BillToLocationID = $billing_id;
	} else if ( $billing_address1 != "" and $billing_city != "" and 
			$billing_state!= "" and $billing_zip != "" ) {
		$sql = "SELECT * FROM customer_addresses WHERE address1 = '$billing_address1'
			AND city = '$billing_city' AND state='$billing_state' AND zip='$billing_zip'";
		$result = mysql_query($sql,$link) or die ( mysql_error() . "Failed Execute SQL : $sql <br />");
		if ( mysql_num_rows($result) > 0 ) {
			$row = mysql_fetch_array($result);
			$BillToLocationID = $row['address_id'];
		} else {
	...
	if ( $shipping_id != "" ) {
		$sql = "SELECT * FROM customer_addresses WHERE address_id = $shipping_id";
		$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
		if ( mysql_num_rows($result) > 0 ) 
			$ShipToLocationID = $_POST['shipping_id'];
	} else if ( $shipping_address1 != "" and $shipping_city != "" 
		and $shipping_state != "" and $shipping_zip != "" ) {
	  $sql = "SELECT * FROM customer_addresses WHERE address1 = '$shipping_address1'
			AND city = '$shipping_city' AND state='$shipping_state' AND zip='$shipping_zip'";
	  $result = mysql_query($sql,$link) or die ( mysql_error() . "Failed Execute SQL : $sql <br />");
	  if ( mysql_num_rows($result) > 0 ) {
		$row = mysql_fetch_array($result);
		$ShipToLocationID = $row['address_id'];
	  } else {
...
$sql = "SELECT tblsystemdefaultsdetail.ItemDescription 
	...
	USING (ItemID) 
	WHERE ItemID = 10";
$packaging_result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
$i = 0;
while ( $row = mysql_fetch_array($packaging_result) ) {
...
$(document).ready(function(){
	...
	$("#customer[readonly!=readonly]").change(function() {
		...
		//{
			...
			$("#billing_state").val("");
			$("#billing_zip").val("");
			$("#billing_address").unautocomplete();
			...
			$("#shipping_state").val("");
			$("#shipping_zip").val("");
			$("#Shipping_address").unautocomplete();
		//}
	});
	$("#customer[readonly!=readonly]").result(function(event, data, formatted) {
		if (data) {
			...
			$("#billing_address[readonly!=readonly]").autocomplete("search/address_by_customer_id.php", {
				cacheLength: 1,
				matchContains: true,
				mustMatch: false,
				minChars: 0,
			...
			$("#billing_address[readonly!=readonly]").result(function(event, data, formatted) {
				if (data) {
					$("#billing_id").val(data[1]);
					$("#billing_address1").val(data[2]);
					$("#billing_address2").val(data[3]);
					$("#billing_city").val(data[4]);
					$("#billing_state").val(data[5]);
					$("#billing_zip").val(data[6]);
				} 
			});
			
			$("#shipping_address[readonly!=readonly]").autocomplete("search/address_by_customer_id.php", {
				cacheLength: 1,
				matchContains: true,
				mustMatch: false,
				minChars: 0,
			...
			$("#shipping_address[readonly!=readonly]").result(function(event, data, formatted) {
				if (data) {
					$("#shipping_id").val(data[1]);
					$("#shipping_address1").val(data[2]);
					$("#shipping_address2").val(data[3]);
					$("#shipping_city").val(data[4]);
					$("#shipping_state").val(data[5]);
					$("#shipping_zip").val(data[6]);
				} 
			});
	...
	$("#billing_address[readonly!=readonly]").change(function() {
		if ("" == $("#customer").val())
		{
			$("input#billing_address").flushCache();
			$("input#billing_address").search();
			$("#billing_address").val("");
			$("#billing_id").val("");
			$("#billing_address1").val("");
			$("#billing_address2").val("");
	...
	$("#shipping_address[readonly!=readonly]").change(function() {
		if ("" == $("#customer").val())
		{
			$("input#shipping_address").flushCache();
			$("input#shipping_address").search();
			$("#shipping_address").val("");
			$("#shipping_id").val("");
			$("#shipping_address1").val("");
			$("#shipping_address2").val("");
	...
	if ( $bsn != '' ) {
		if ( $pne != '' ) {
			...
			//if  ( 1 == $row_pni[Intermediary] ) 
			{
				...
				$c = mysql_num_rows($result_ipni);
				if ( $c < 1 ) {
					die("Could not find linked product to this Intermediary product ($sql). Please check if the product type is correct.<br />");
				}
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
				...
				$sql = "SELECT * FROM customerorderdetail ".
                       "LEFT JOIN batchsheetcustomerinfo on customerorderdetail.CustomerOrderNumber = batchsheetcustomerinfo.CustomerOrderNumber ".
                       "WHERE ShipDate IS NULL AND BilledDate IS NULL ".
                       " AND customerorderdetail.CustomerOrderNumber = '". $row['OrderNumber'] . "' ".
                       " AND not exists (select * from inventorymovements , batchsheetdetailpackaginglotnumbers where inventorymovements.transactionnumber = batchsheetdetailpackaginglotnumbers.inventorymovementtransactionnumber ".
                       " AND batchsheetdetailpackaginglotnumbers.batchsheetnumber=batchsheetcustomerinfo.batchsheetnumber) ".
                       " AND not exists (SELECT * FROM customerorderdetail as cod where cod.CustomerOrderNumber = customerorderdetail.CustomerOrderNumber and ( cod.shipdate is not null or cod.billeddate is not null))";
...
<?php if ( $order_num != '' or $action == 'edit' ) { 
	...
	if ( $order_num != "" ) { // e-mail to customer or check confirmed to customer'
	
	$sql = "SELECT * FROM customerordermaster WHERE OrderNumber = '" . $order_num . "'";
	$result=mysql_query($sql,$link) or die ( mysql_error() ." Failed execute SQL : $sql <br />");;
	$row=mysql_fetch_array($result);
	$sql_cntct="SELECT first_name,last_name,phone_id,title,number, phone_types.description FROM customer_contacts
	LEFT JOIN customer_contact_phones USING(contact_id)
	LEFT JOIN phone_types ON customer_contact_phones.type=phone_types.type_id
	WHERE customer_contacts.customer_id='".$row['CustomerID']."' AND phone_id is not null";
	//echo "<br /> $sql_cntct <br />";
	
	$result = mysql_query($sql_cntct,$link) or die ( mysql_error() . " FAILED To EXECUTE SQL : $sql_cntct <br />");
	$directContactOpt="<SELECT NAME='contact_phone_fax'>";
	while ( $row_cntct = mysql_fetch_array($result) ) {
	  $directContactOpt .= "<OPTION value='".$row_cntct['phone_id']."'>". $row_cntct['description']." ".escape_data($row_cntct['number']).
		" " .$row_cntct['first_name'] ." ".$row_cntct['last_name'] ." ".$row_cntct['title']."</OPTION>";
		}
	$directContactOpt .= "</SELECT>";	
 	
	}
	
?>
<br />
	<div id="customerOrderHeadBar" style="background-color:<?php echo ($shipping_status == "Complete") ? "lawngreen":"#FFF999" ?>;width:890px; padding:0.5em 0; margin-bottom:0.5em; font-size:1em ; text-align:center"><b>Status: </b><?php echo ($shipping_status > 0) ? "Completed": "Incomplete" ?>

	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<b>OrderNumber: </b> <?php echo $order_num;?>
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<?php if ( $row['ConfirmedToCustomer'] ) { ?>
	
	<INPUT TYPE="checkbox" NAME="custordConfirmed" ID="custordConfirmed" VALUE="1" readonly="readonly" CHECKED> <label for="custordConfirmed"><?php echo $row['ConfirmedBy'];?></lable>
	<INPUT TYPE="button" value="View Confirm File" onClick="popup('<?php echo $row['ConfirmFile'];?>')">
	&nbsp;<INPUT TYPE="button" value="Resend Confirm" onClick="resend_confirm('<?php echo $row['OrderNumber'];?>')">
	<?php } else if ( !$shipping_status == "Incomplete" ) { ?>
	<b>Send Confirmation to Customer</b>
	&nbsp;

	<INPUT TYPE="button" VALUE="By Direct Contact" id="directContactbutton" onClick="OpenDirectContact();">
	&nbsp;

	<INPUT TYPE="button" VALUE="By E-Mail" onClick="popup('customers_order.email.php?order_number=<?php echo $row['OrderNumber'];?>&direct_contact=0')">
	<?php } ?>
	</div>
	<br />
		...
		<TR VALIGN=TOP>
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="3">
				...
				<TR>
					...
					<TD><INPUT TYPE="text" NAME="CustomerPONumber" id="CustomerPONumber" VALUE="<?php echo $CustomerPONumber?>" SIZE="26" <?php echo $form_status;?>>
					</TD>
				</TR>
			...
			<TABLE BORDER="0" CELLPADDING="3" CELLSPACING="0">
				...
				<TR>
					...
					<?php if ( $C_of_A_Requested == 1 ) { ?>
						<INPUT TYPE="checkbox" NAME="C_of_A_Requested" ID="C_of_A_Requested" VALUE="1" CHECKED <?php echo $form_status;?>/>
					<?php } else { ?>
						<INPUT TYPE="checkbox" NAME="C_of_A_Requested" ID="C_of_A_Requested" VALUE="1" <?php echo $form_status;?>/>
					<?php } ?>
					</TD>
					<TD><label for="C_of_A_Requested">C of A Requested</label></TD>
					</TR>
					<TR>
					<TD>
					<?php if ( $MSDS_Requested == 1 ) { ?>
						<INPUT TYPE="checkbox" NAME="MSDS_Requested" ID="MSDS_Requested" VALUE="1" CHECKED <?php echo $form_status;?>/>
					<?php } else { ?>
						<INPUT TYPE="checkbox" NAME="MSDS_Requested" ID="MSDS_Requested" VALUE="1" <?php echo $form_status;?>/>
					<?php } ?>
					</TD>
					<TD><label for="MSDS_Requested">MSDS Requested</label></TD>
					</TR>
					<TR>
					<TD>
					<?php if ( $NAFTA_Requested == 1 ) { ?>
						<INPUT TYPE="checkbox" NAME="NAFTA_Requested" ID="NAFTA_Requested" VALUE="1" CHECKED <?php echo $form_status;?>/>
					<?php } else { ?>
						<INPUT TYPE="checkbox" NAME="NAFTA_Requested" ID="NAFTA_Requested" VALUE="1" <?php echo $form_status;?>/>
					<?php } ?>
					</TD>
					<TD><label for="NAFTA_Requested">NAFTA Requested</label></TD>
					</TR>
					<TR><TD>

					<?php if ( $Hazardous_Info_Requested == 1 ) { ?>
						<INPUT TYPE="checkbox" NAME="Hazardous_Info_Requested" ID="Hazardous_Info_Requested" VALUE="1" CHECKED <?php echo $form_status;?>/>
					<?php } else { ?>
						<INPUT TYPE="checkbox" NAME="Hazardous_Info_Requested" ID="Hazardous_Info_Requested" VALUE="1" <?php echo $form_status;?>/>
					<?php } ?>
					</TD>
					<TD><label for="Hazardous_Info_Requested"><NOBR>Hazardous Info Requested</NOBR></label></TD>
					<TR>
					<TD>
					<?php if ( $Kosher == 1 ) { ?>
						<INPUT TYPE="checkbox" NAME="Kosher" ID="Kosher" VALUE="1" CHECKED <?php echo $form_status;?>/>
					<?php } else { ?>
						<INPUT TYPE="checkbox" NAME="Kosher" ID="Kosher" VALUE="1" <?php echo $form_status;?>/>
					<?php } ?>
					</TD>
					<TD><label for="Kosher">Kosher</label></TD>
				</TR>
			</TABLE>
			</TD>
			<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
			...
			<TABLE BORDER="0" CELLPADDING="3" CELLSPACING="0">
				...
				<?php if (""==$form_status) { ?>
				<TR>
					<TD STYLE="text-align:right"><B>Choose Address:</B></TD>
					<TD><INPUT TYPE="text" name="billing_address" id="billing_address" size="26"></TD>
				</TR><?php } ?>
				<INPUT TYPE="hidden" NAME="billing_id" id="billing_id" VALUE="<?php echo $BillToLocationID;?>">
				<TR>
					<TD STYLE="text-align:right"><B>Address 1:</B></TD>
					<TD><INPUT TYPE="text" NAME="billing_address1" id="billing_address1" VALUE="<?php echo $billing_address1;?>" SIZE="26" <?php echo $form_status;?>></TD>
				</TR>
				<TR>
					<TD STYLE="text-align:right"><NOBR><B>Address 2:</B></NOBR></TD>
					<TD><INPUT TYPE="text" NAME="billing_address2" id="billing_address2" VALUE="<?php echo $billing_address2;?>" SIZE="26" <?php echo $form_status;?>></TD>
				</TR>
				<TR>
					<TD STYLE="text-align:right"><B>City:</B></TD>
					<TD><INPUT TYPE="text" NAME="billing_city" id="billing_city" VALUE="<?php echo $billing_city;?>" SIZE="26" <?php echo $form_status;?>></TD>
				</TR>
				<TR>
				...
				<TR>
					...
					<TD><INPUT TYPE="text" NAME="billing_zip" id="billing_zip" VALUE="<?php echo $billing_zip;?>" SIZE="15" <?php echo $form_status;?>></TD>
				</TR>
			</TABLE>
			</TD>
			<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
			...
			<TABLE BORDER="0" CELLPADDING="3" CELLSPACING="0">
				...
				<TR>
					<TD STYLE="text-align:right"><B>Choose Address:</B></TD>
					<TD><INPUT TYPE="text" id="shipping_address" NAME="shipping_address" size="26"</TD>
				</TR>
				<TR>
				...
				<?php } ?>
				<INPUT TYPE="hidden" NAME="shipping_id" id="shipping_id" VALUE="<?php echo $ShipToLocationID;?>">
					
				<TR>
					<TD STYLE="text-align:right"><B>Address 1:</B></TD>
					<TD><INPUT TYPE="text" NAME="shipping_address1" id="shipping_address1" VALUE="<?php echo $shipping_address1;?>" <?php echo $form_status;?> SIZE="26"></TD>
				</TR>
				<TR>
					<TD STYLE="text-align:right"><NOBR><B>Address 2:</B></NOBR></TD>
					<TD><INPUT TYPE="text" NAME="shipping_address2" id="shipping_address2" VALUE="<?php echo $shipping_address2;?>" <?php echo $form_status;?> SIZE="26"></TD>
				</TR>
				<TR>
					<TD STYLE="text-align:right"><B>City:</B></TD>
					<TD><INPUT TYPE="text" NAME="shipping_city" id="shipping_city" VALUE="<?php echo $shipping_city;?>" SIZE="26" <?php echo $form_status;?>></TD>
				</TR>
				<TR>
					<TD STYLE="text-align:right"><B>State:</B></TD>
				...
				<TR>
					<TD STYLE="text-align:right"><B>ZIP:</B></TD>
					<TD><INPUT TYPE="text" NAME="shipping_zip" id="shipping_zip" VALUE="<?php echo $shipping_zip;?>" SIZE="15" <?php echo $form_status;?>></TD>
				</TR>
			</TABLE>
				</TD>
			</TR>
...
<TABLE CELLPADDING=2 CELLSPACING=0 BORDER=0 BGCOLOR="#9966CC"><TR><TD>
			...
			<TR>
				...
				<TABLE ALIGN=RIGHT BORDER=0 CELLSPACING="0" CELLPADDING="3" BORDERCOLOR="#CDCDCD">
					...
					<TR>
						...
						<TD><select class="input-box" name="Packaged_In">
						<OPTION VALUE=""></OPTION>
						<?php
							$pkinselected=false;
							if ( $Packaged_In != "" )
							...
							foreach ( $packaging as $value ) {
								if ( $value == $Packaged_In ) {
									echo "<OPTION VALUE='" . $value . "' SELECTED>" . $value . "</OPTION>";
								} else if ( ! $pkinselected and substr($value,0,13) == "5-Gallon Pail" ) {
									echo "<OPTION VALUE='" . $value . "' SELECTED>" . $value . "</OPTION>";
								} else {
									echo "<OPTION VALUE='" . $value . "'>" . $value . "</OPTION>";
								}
 ...
 -->
	...
	$sql = "SELECT customerorderdetail.*, externalproductnumberreference.ProductNumberExternal FROM customerorderdetail
	INNER JOIN externalproductnumberreference USING (ProductNumberInternal) 
	WHERE CustomerOrderNumber = '" . $order_num . "' ORDER BY CustomerOrderSeqNumber";
	$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	//echo $sql . "<BR>";
	$c = mysql_num_rows($result);
	if ( $c > 0 ) { ?>
		...
		<TABLE ALIGN=RIGHT BORDER=1 CELLSPACING="0" CELLPADDING="3" BORDERCOLOR="#CDCDCD">
			...
			<TR VALIGN=BOTTOM>
				...
				<TD ALIGN=CENTER><B>Pack In</B></TD>
				<!-- <TD><B>Vendor</B></TD>
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			<INPUT TYPE="hidden" NAME="edit_prod" VALUE="1">
				...
				<TD><select class="input-box" <?php echo (""==$prod_form_status ? "id=\"Packaged_In\" ":"") ?> name="Packaged_In" <?php echo $prod_form_status;?>>
				<OPTION VALUE=""></OPTION>
				<?php
				$sql="select Packaged_In from pricesheetmaster left join customerordermaster on pricesheetmaster.CustomerID=customerordermaster.CustomerID
				where customerordermaster.OrderNumber='" . $order_num . "' and pricesheetmaster.ProductNumberinternal='".$row['ProductNumberInternal']."' and locked>0";
				$pk_result=mysql_query($sql,$link) or die ( mysql_error() ." Failed execute SQL: $sql<br/>");
				if ( mysql_num_rows($pk_result) == 0 ) { //wided filter
					$sql="select Packaged_In from pricesheetmaster left join customerordermaster on pricesheetmaster.CustomerID=customerordermaster.CustomerID
					where customerordermaster.OrderNumber='" . $order_num . "' and pricesheetmaster.ProductNumberinternal='".$row['ProductNumberInternal']."' AND SellingPrice>0";
					$pk_result=mysql_query($sql,$link) or die ( mysql_error() . " Failed Execute SQL : $sql <br />");
				}
				$row_packin = mysql_fetch_array($pk_result);
				
				foreach ( $packaging as $value ) {
					if ( $value == $row['OrderPackIn'] ) {
						echo "<OPTION VALUE='" . $value . "' SELECTED>" . $value . "</OPTION>";
					} else if ( $row['OrderPackIn'] == "" and $value == $row_packin[0] ) {
						echo "<OPTION VALUE='" . $value . "' SELECTED>" . $value . "</OPTION>";
					} else {
						echo "<OPTION VALUE='" . $value . "'>" . $value . "</OPTION>";
					}
...
<TD>
 ...
 -->
        ...
        <form>
		  ...
		  &nbsp;&nbsp;&nbsp;&nbsp;
		  <INPUT TYPE="button" CLASS="submit" VALUE="Print Pack Labels" onClick="popup('reports/print_customer_order_labels.php?order_num=<?php echo $order_num;?>',820,830)">
		  <?php if ( $may_dlt_order ) { ?>
           &nbsp;&nbsp;&nbsp;&nbsp;
		  <INPUT TYPE="button" CLASS="submit" VALUE="Delete Order" onClick="delete_order(<?php echo $order_num; ?>)">
          <?php } ?>
		</form>
        </NOBR></div>
...
function delete_order(cid) {
	if ( confirm('Are you sure you want to delete this order? Order#:' + cid) ) {
		document.location.href = "customers_customer_order_shipping.php?action=delete&cid=" + cid;
	}
}

function OpenDirectContact() {
	document.getElementById('customerOrderHeadBar').innerHTML=
	"<FORM action='confirmCustomerOrder.php' method='post' target='_blank'>" + 
	"<INPUT type='hidden' name='order_number' value='<?php echo $order_num;?>'>\n" +
	"SELECT Phone or Fax Number# <?php echo $directContactOpt;?>" +
	"<input type='submit' value='submit'>&nbsp;&nbsp;<input type='button' value='Cancel' onClick='window.location.reload();'>" +
	"</FORM>";
}

function resend_confirm(ordernumber) {
	document.getElementById('customerOrderHeadBar').innerHTML=
	"<INPUT TYPE='button' VALUE='By Direct Contact' id='directContactbutton' onClick='OpenDirectContact();'>&nbsp;"+
	"<INPUT TYPE='button' VALUE='By E-Mail' onClick=\"popup(\'customers_order.email.php?order_number="+ordernumber+"&direct_contact=0\')\">";
}
