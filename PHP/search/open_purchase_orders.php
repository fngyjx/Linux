...
$sql .= "WHERE `purchaseorderdetail`.`ProductNumberInternal` NOT LIKE '7%' AND `purchaseorderdetail`.`IntermediarySentToVendor` = 0 AND `purchaseorderdetail`.`Status` IN ('O','P','') 
	AND `purchaseorderdetail`.`TotalQuantityExpected` > 0 AND (`purchaseordermaster`.`PurchaseOrderType` <> 'Process' or `purchaseordermaster`.`PurchaseOrderType` is null)";
...
//	print_r($row); 
		echo "PO:".$row["PurchaseOrderNumber"]."(".$row["PurchaseOrderSeqNumber"].")".
		" - Prod#:".$row["ProductNumberInternal"]." - Desig.:".$row["Designation"].
		" - Vendor:".$row["name"]." - Quantity:".$row["Quantity"]." - Pack Size:".$row["PackSize"].
		" - Units:".$row["UnitOfMeasure"]." - Unit Price:".$row["UnitPrice"]." - Tot. Ordered:".$row["TotalQuantityOrdered"].
		" - Tot. Received:".$row['total_received_native']."|".$row["PurchaseOrderNumber"]."|".
		$row["PurchaseOrderSeqNumber"]."|".$row["ProductNumberInternal"]."|".$row["Designation"]."|".$row["vendor_id"].
		"|".$row["name"]."|".$row["VendorProductCode"]."|".$row["OpenQuantity_native"]."|".$row["PackSize"].
		"|".$row["UnitOfMeasure"]."|".$row["ID"]."\n";
