...
if(""!=$x_id){
	$sql = "SELECT TransactionDate, Quantity, TransactionType, Remarks, LotNumber, LotSequenceNumber , 
					...
					WHERE pm.ProductNumberInternal=inventorymovements.ProductNumberInternal AND lots.ID=inventorymovements.LotID 
					AND TransactionNumber=$x_id LIMIT 1";
...
// validate
	...
	if ( "lbs"!=$units && "grams" != $units && "kilos"!=$units && "N/A"!=$units)
		$error_message .= "Units Error ($units) - please edit the materials page for this item and set the units.<br/>";
	/* jdu 111109 if ( ""==$quantity || 0 >= $quantity)
...
function validated() {
	...
	/* jdu 111109 if ("" == $("#quantity").val() || 0 > $("#quantity").val() ) {
