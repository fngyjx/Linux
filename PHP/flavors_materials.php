...
if (isset($_REQUEST['external_number_search'])) {
	$tmpArr = explode("&nbsp;", $_REQUEST['external_number_search']);
	$external_number_search=$tmpArr[0];
...
if (isset($_REQUEST['internal_number_search'])) {
	$tmpArr = explode("&nbsp;",$_REQUEST['internal_number_search']);
	$internal_number_search=$tmpArr[0];
...
if ( $action == 'search' ) {
	...
	$sql .= "productmaster.Natural_OR_Artificial, productmaster.Designation, productmaster.ProductType, productmaster.Kosher, 
		externalproductnumberreference.ProductNumberExternal, productmaster.Notes ";
	...
	$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	search_list($result,"master");
	
	$clause=str_replace("productmaster","deleted_productmaster",$clause);
	
	$sql = "SELECT DISTINCT deleted_productmaster.ProductNumberInternal, deleted_productmaster.SpecificGravity, deleted_productmaster.SpecificGravityUnits, deleted_productmaster.Organic, ";
	$sql .= "deleted_productmaster.Natural_OR_Artificial, deleted_productmaster.Designation, deleted_productmaster.ProductType, deleted_productmaster.Kosher, 
		deleted_externalproductnumberreference.ProductNumberExternal, deleted_productmaster.Notes ";
	$sql .= "FROM deleted_externalproductnumberreference RIGHT JOIN deleted_productmaster ON deleted_externalproductnumberreference.ProductNumberInternal = deleted_productmaster.ProductNumberInternal ";
	$sql .= "WHERE ( 1 ". $clause  ." ) ";
	$sql .= "ORDER BY deleted_productmaster.ProductNumberInternal";
	
	// echo "<br /> $sql <br />";
	$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	
	search_list($result,"deleted");
...
?>
		...
		<table style="padding-left:1em; float: left;" width="400" border="0" cellpadding="0" cellspacing="0">
			<tbody>
			<TR VALIGN=TOP>
				...
				<?php if ( $Packaging == "" ) {
					...
					if ( $c > 0 ) {
						...
						while( $row = mysql_fetch_array($result) ) {
						 if( $row['DefaultPksz'] ) {
							$default_found=true;
							echo $row['PackagingType'];
						?>&nbsp;<INPUT type="button" value="edit" onClick="popup('pop_select_product_packsize.php?action=edit&pni=<?php echo $pni; ?>',500,400)">
					<?php } }
					if ( ! $default_found ) { ?>
						<INPUT type="button" value="Set Default" onClick="popup('pop_select_product_packsize.php?action=edit&pni=<?php echo $pni; ?>',500,400)">
					<?php
					}
				}else {
						echo "<INPUT TYPE='button' VALUE='Add' onClick='popup(\"pop_select_product_packsize.php?action=add&pni=".$pni."\",500,400)'>";
					}
				} else { ?>
				<INPUT TYPE="text" NAME="Packaging" VALUE="<?php echo $Packaging;?>" SIZE="27">
				<INPUT type="button" value="edit" onClick="popup('pop_select_product_packsize.php?action=edit&pni=<?php echo $pni; ?>',700,830)">
				<?php } ?>
				</TD>
			</TR>
			...
			<TR VALIGN=TOP>
				...
				<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="5" HEIGHT="1" BORDER="0"></TD>
				<TD><INPUT TYPE="text" NAME="QuickScan" VALUE="<?php echo $QuickScan;?>" SIZE="27" maxlength="45"></TD>
			</TR>
		...
		<INPUT style="float:right" TYPE="button" id="formula_list" name="formula_list" VALUE="Formula Listing" CLASS="submit_medium" 
		onClick="popup('material_formulas_listing.php?pni=<?php echo $pni;?>')"/>
<?php
...
<?php
  ...
  function search_list($result,$data_source) {
	$c = mysql_num_rows($result);
	//echo $sql . "<BR>";
	...
	if ( $data_source != "master" ) {
		...
		echo "<div id='removed_prd_0' style='display:inline'><h5> deleted material list</h5><div>";
    }				
		$bg = 0; ?>

		<FORM id="edit_<?php echo $data_source;?>">

		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="2" width="100%">
		<?php if ($data_source == "deleted" ) { ?>
		<!-- <TR><TD colspan='18'>
		<input type="button" value="Show Deleted Materials" onClick="showDeleted(<?php echo $c;?>)"></TD></TR>
		<div id="removed_prd_1" style='display:none'> -->
		<?php $i=2; 
		}?>
			<TR VALIGN=BOTTOM>
				<TD>&nbsp;</TD>
				...
				<TD><B>SG Units</B></TD>
				<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
				<TD><B>View<br />/Edit</B></TD>
				<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
			<!--	<TD colspan='3'><B>Inventory</B></TD>
				<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD> -->
				<TD><B>Formula<br />Listing</B></TD>
			</TR>

			<TR>
				<TD COLSPAN='18'><IMG SRC="images/spacer.gif" WIDTH="1" HEIGHT="7"></TD>
			</TR>
			...
			while ( $row = mysql_fetch_array($result) ) {
				...
				<TD>
				<?php 
				if ( $_SESSION['user_id'] == '4' or $_SESSION['user_id'] == '5' or $_SESSION['user_id'] == '35' ) {
					if($data_source == 'deleted' ) { 
				//	echo "<div id='removed_prd_".$i."' style='display:none'>";
				//	echo $row['Notes'];
					
				?>
					<A href="javaScript:RollBack_dltMrgPrd('<?php echo $row['ProductNumberInternal'];?>')"><IMG src="/images/return.gif" width="10px" height="10px"></A>
					
				<?php 
				// echo "</div>";
				
					} else { ?>
					<A href="javaScript:dltMrgPrd('<?php echo $row['ProductNumberInternal'];?>')"><IMG src="/images/delete.gif"></A>
				<?php }
				} else { echo "&nbsp;"; }?>
					</TD>
					...
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<TD style="font-size:9px;"><?php echo number_format($row['SpecificGravity'],2) ?></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<TD style="font-size:9px;"><?php echo $row['SpecificGravityUnits'] ?></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<?php if ( $data_source == "master" ) { ?>
					<TD><A href="flavors_materials.php?action=edit&ProductNumberInternal=<?php echo $row['ProductNumberInternal'];?>"><img src="images/zoom.png"></A></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
				<!--	<TD><INPUT TYPE="button" style="font-size:9px;" VALUE="Location" onClick="popup('reports/inventory_reports_by_location.php?pni=<?php echo $row['ProductNumberInternal'];?>')"></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<TD><INPUT TYPE="button" style="font-size:9px;" VALUE="Movements" onClick="popup('reports/inventory_reports_movements_list.php?pni=<?php echo $row['ProductNumberInternal'];?>')"></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD> -->
					<TD><INPUT TYPE='button' style="font-size:9px;" VALUE="Formula Listing" onClick="popup('material_formulas_listing.php?pni=<?php echo $row['ProductNumberInternal'];?>')" target="_blank"></TD>
					<?php } else if ( $data_source== "deleted" ) {?>
					<TD><A href="flavors_materials.php?action=edit_deleted&ProductNumberInternal=<?php echo $row['ProductNumberInternal'];?>"><img src="images/zoom.png"></A></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<TD>&nbsp;</TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
					<TD colspan='3'>&nbsp;</TD>
...
<?php// if ( $data_source == "deleted" ) echo "</div>";?>
		</TABLE>
...
function dltMrgPrd(pni) {
//1. ask user for sure to delete/merge the product
//2. if this is a merging action, prompt for an internal# that merging to
//3. redirect/popup material delete process

//in material delete process, 1. prompt for password, 2. check the deletable? (if open BS, cstpo, etc. exists)
//and give the statement that the delete will only affect future and before now will keep same
//3. merge process will be allowed without condition limit

//1.2 
	if ( confirm("Are you sure you want to remove " + pni + " ? The activity need PassWord. If you don't have the PassWord, Stop Here") ) {
		var mergtopni = prompt("If this action is to Merge " + pni + " To another material, please provide the material # now, Otherwise the " + pni + " will be deleted if the condition allow.",'');
		popup("secure/pop_material_delete.php?pni="+pni+"&mergtopni="+mergtopni);
	}
	

}

function RollBack_dltMrgPrd(pni) {
//we may not need this function for rollback if we don't need to warning the user
	if ( confirm("Are you sure you want to replace  " + pni + " ? The activity need PassWord. If you don't have the PassWord, Stop Here") ) {
		//var mergtopni = prompt("If this action is to Merge " + pni + " To another material, please provide the material # now, Otherwise the " + pni + " will be deleted if the condition allow.");
		popup("secure/pop_material_rollback.php?pni="+pni);
	}
	

}

function showDeleted($items) {
// items is the total of materials in the lists, loop through all in the list and display the hidden ones
	for ( var i=0; i<$items; i++) {
		var myElm = document.getElementById("removed_prd_"+i);
		if ( myElm ) {
