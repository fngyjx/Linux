...
if ( $_GET['action'] == "inact" ) {
	...
	mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	header("location: customers_customers.php?alpha=".$_GET['alpha']);
}
...
<?php if ( $_GET['action'] == "" ) { ?>
	...
	if ( $_GET['alpha'] ) {
		...
		$sql = "SELECT DISTINCT customer_id, name FROM customers LEFT JOIN customers_users USING(customer_id) WHERE active=1 AND " . $alpha_clause . $name_clause . " ORDER BY name";
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1">
				...
				while ( $row = mysql_fetch_array($result) ) {
					...
					<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
						...
						<?php } ?>
							<INPUT TYPE="button" VALUE="Delete" onClick="inactivate('<?php echo $row['customer_id'].'\',\''. $alpha .'\',\''.$row['name'];?>')" STYLE="font-size:7pt">
						</TD>
...
function inactivate(cid,alpha, name) {
	if ( confirm('Are you sure you want to remove customer '+ name +'?') ) {
		document.location.href = "customers_customers.php?action=inact&cid=" + cid+"&alpha="+alpha;
	}
}
