...
if ( $_GET['action'] == "delete_tier" ) {
	$tmpArr = explode("&", $_GET['ProductNumberInternal']);
	$sql = "UPDATE productprices SET is_deleted = 1 WHERE VendorID = " . $_GET['VendorID'] . " AND ProductNumberInternal = '" . $tmpArr[0] . "' AND Tier = '" . $_GET['Tier'] . "'";
...
$result_outer = mysql_query($sql_outer, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	...
	while ( $row_outer = mysql_fetch_array($result_outer) ) {
	  if ( "4" == substr($row_outer['IngredientProductNumber'],0,1) or "10829" == substr($row_outer['IngredientProductNumber'],0,5) )
		continue;
		...
		WHERE 1=1 AND (is_deleted=0 or is_deleted is null) " . $ProductNumberInternal_clause;
