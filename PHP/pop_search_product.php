...
<FORM ACTION="pop_search_product.php" name="search" id="search" METHOD="post">
...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
	...
	<TR>
		...
		<TD><INPUT TYPE="text" NAME="Designation" id="Designation" VALUE="<?php echo $Designation;?>" SIZE="30"></TD>
	</TR>
	...
	<TR>
		...
		<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
		<TD><INPUT TYPE="text" NAME="ProductNumberExternal" id="ProductNumberExternal" VALUE="<?php echo $ProductNumberExternal;?>" SIZE="30" readonly='readonly'></TD>
	</TR>
	...
	<TR>
		...
		<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
		<TD><INPUT TYPE="text" NAME="ProductNumberInternal" id="ProductNumberInternal" VALUE="<?php echo $ProductNumberInternal;?>" SIZE="30"></TD>
	</TR>
	...
	<TR>
		<TD><B>Keywords:</B></TD>
		<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
		<TD><INPUT TYPE="text" NAME="Keywords" id="Keywords" VALUE="<?php echo $Keywords;?>" SIZE="30"></TD>
	</TR>
...
if ("" != $VendorID) { 
	...
	// $sql = "SELECT DISTINCT pp.ProductNumberInternal as pni, pm.*, epnr.ProductNumberExternal FROM productprices AS pp, productmaster as pm LEFT JOIN externalproductnumberreference AS epnr ON ( epnr.ProductNumberInternal = pm.ProductNumberInternal ) WHERE pm.ProductNumberInternal = pp.ProductNumberInternal $VendorClause $Designation_clause $ProductNumberExternal_clause  $ProductNumberInternal_clause $Keywords_clause";
		$sql = "SELECT DISTINCT pm.ProductNumberInternal as pni, pm.*, epnr.ProductNumberExternal 
		...
		WHERE 1 $VendorClause $Designation_clause $ProductNumberExternal_clause  $ProductNumberInternal_clause $Keywords_clause";
...
$(document).ready(function(){
	$("#Designation").autocomplete("search/product_price_by_designation.php?VendorID=<?php echo $VendorID;?>", {
		cacheLength: 1,
		width: 650,
		max: 50,
		scroll: true,
	...
	$("#Designation").keypress(function(e) {
		if (e.which >= 32) {
			$("#ProductNumberExternal").val('');
			$("#ProductNumberInternal").val('');
			$("#Keywords").val('');
			$("#action").val('search');
		}
	});
	$("#ProductNumberExternal").autocomplete("search/product_price_by_external_number.php", {
		cacheLength: 1,
		selectFirst: false,
		matchContains: true,
		mustMatch: false,
		...
		if (data) {
		  ...
		  var rcrdindx = datastr.indexOf('&nbsp;');
		  datastr=datastr.substr(0,rcrdindx);
		  $("#ProductNumberExternal").val(datastr);
		  document.search.submit();
		}
	});
	$("#ProductNumberExternal").keypress(function(e) {
		if (e.which >= 32) {
			$("#Designation").val('');
			$("#ProductNumberInternal").val('');
			$("#Keywords").val('');
	...
	$("#ProductNumberInternal").autocomplete("search/product_price_by_internal_number.php?VendorID=<?php echo $VendorID;?>", {
		cacheLength: 1,
		selectFirst: false,
		matchContains: true,
		mustMatch: false,
	...
	$("#ProductNumberInternal").keypress(function(e) {
		if (e.which >= 32) {
			$("#Designation").val('');
			$("#ProductNumberExternal").val('');
			$("#Keywords").val('');
	...
	$("#Keywords").autocomplete("search/product_price_by_keyword.php", {
		cacheLength: 1,
		selectFirst: false,
		matchContains: true,
		mustMatch: false,
