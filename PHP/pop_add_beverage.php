...
if ( ("clone_submit" == $action) || ("save" == $action) )  {
	...
	/* Validate */
	if( validateData( $external_number, $intermediary, $designation, $quick_scan, $n_or_a, $product_type, $kosher, $organic, $inventory_units, $error_message) ) {
		$date="";
...
function validateData( &$external_number, &$intermediary, &$designation, &$quick_scan, &$n_or_a, &$product_type, &$kosher, &$organic, &$inventory_units, &$error_message = "" ) {
	$valid = true;
	global $link;
...
<h1>Add New Beverage Product Record</h1>
<?php 
//echo "<h2 style=\"color:red\">Action is $action</h2>"; 
?>

<FORM id="similar_record_search" name="similar_record_search" ACTION="pop_add_beverage.php" METHOD="post">
...
<?php 	
	if ( $_REQUEST[product_designation_search] != "" ) {
?>
...
<?php
		...
		$sql .= "WHERE productmaster.ProductNumberInternal LIKE '8%' and ProductNumberExternal != '' $search_record ";
...
?>
	...
	<tbody>
	<tr><td>
		<div class="type_formula" >
			<h2>New Beverage Formula Record Fields</h2>
		</div>
		<div class="type_formula">
		...
		<div class="type_formula type_rawMaterial">
