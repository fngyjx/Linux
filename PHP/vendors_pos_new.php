...
<BR><BR>
	...
	if ( $_REQUEST['action'] != 'search' ) {
		...
		LEFT JOIN purchaseorderdetail USING (PurchaseOrderNumber)
        WHERE purchaseorderdetail.Status in ('O', 'P','') OR purchaseorderdetail.Status IS NULL  
        OR purchaseordermaster.PurchaseOrderNumber NOT IN (SELECT PurchaseOrderNumber FROM purchaseorderdetail)
			...
			<INPUT TYPE="hidden" NAME="edit_prod" VALUE="1">
				...
				<?php if ( "" != $form_status ) { ?>
					...
					<?php //use editable dropdow or show hint? hint may be better.
							$sql = "SELECT vendor_name VendorName, ProductNumberInternal,PriceEffectiveDate,PricePerPound,
				...
				<?php if ( isset($sql_prdprice) ) { // print hint for po price and units
					...
					<TABLE>
						...
						<TH>Package</TH><TH bgcolor='white'>Min.</TH><TH>Notes</TH></TR>
					<?php
					while ( $row_prdhint = mysql_fetch_array($sql_prdprice) ) {
						...
						echo "<TD>" . $row_prdhint['Kosher'] ." ". $row_prdhint['Notes'] ."</TD>";
						echo "</TR>";
					}
					echo "</TABLE></TD></TR>";
				} ?>
