...
<TABLE class="bounding"><TR valign="top"><TD class="padded">
	...
	<?php if ( $_GET['alpha'] or $_POST['action'] == "search" ) { ?>
		...
		if ( $_POST['first'] != '' or $_POST['last'] != '' or $_POST['customer'] != '' ) {
			...
			$sql = "SELECT DISTINCT contact_id, vendor_contacts.vendor_id, name, first_name, last_name, vendor_contacts.active FROM vendor_contacts LEFT JOIN vendors USING(vendor_id) WHERE " . $id_filter . $first_filter . $last_filter . " ORDER BY last_name";

		} else {
			$sql = "SELECT DISTINCT contact_id, vendor_contacts.vendor_id, name, first_name, last_name, vendor_contacts.active FROM vendor_contacts LEFT JOIN vendors USING(vendor_id) WHERE last_name LIKE '$alpha%' " . $active_filter . " ORDER BY last_name";
		}
