...
$sql = "SELECT * FROM projects LEFT JOIN customer_contacts USING(contact_id) LEFT JOIN customers USING(customer_id) WHERE " . $date_filter . " AND (status > 2 AND status < 5) " . $filter;
...
if ( mysql_num_rows($result) > 0 ) {
	...
	if ( $_SESSION['userTypeCookie'] == 1 ) {
		$page = "project_management_front_desk.php";
	} else {
		$page = "project_management_admin.php";
	}
	...
	$bg = 0; ?>
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="2">
				<TR>
					...
					<TD><B><A HREF="<?php echo $page;?>?sortby=c&archives=<?php echo $archives;?>">Customer</A></B></TD>
					<TD><IMG SRC="images/spacer.gif" WIDTH="7" HEIGHT="1"></TD>
				...
				while ( $row = mysql_fetch_array($result) ) {
					...
					<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
						...
						<TD><IMG SRC="images/spacer.gif" WIDTH="1" HEIGHT="1"></TD><FORM ACTION="project_management_front_desk_packing_slip.php?pid=<?php echo $row['project_id'] ?>" METHOD="post" TARGET="_blank">
						...
						<?php } else { ?>
							<INPUT TYPE="button" VALUE="Mark as shipped" onClick="window.location='project_management_admin.sales.php?stat=4&pid=<?php echo $row['project_id'];?>'" <?php echo $can_submit ?> STYLE="font-size:7pt">
						<?php } ?>
						</TD></FORM>
					</TR>
