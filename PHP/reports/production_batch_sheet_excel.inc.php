...
$objPHPExcel->getActiveSheet()->mergeCells('A1:'.$last_column.'1');
...
$objPHPExcel->getActiveSheet()->mergeCells('K2:'.$last_column.'2');
...
$objPHPExcel->getActiveSheet()->mergeCells('H3:'.$last_column.'3');

$objPHPExcel->getActiveSheet()->getStyle('D3:'.$last_column.'3')->GetAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);

//$Vessel = " 250-G Jacketed Kettel";
$objPHPExcel->getActiveSheet()->setCellValue('E4', 'Vessel:' . $Vessel );
$objPHPExcel->getActiveSheet()->mergeCells('E4:'.$last_column.'4');
...
$objPHPExcel->getActiveSheet()->mergeCells('E5:'.$last_column.'5');

$objPHPExcel->getActiveSheet()->getStyle('A2:'.$last_column.'5')->applyFromArray($styleMediumBlackBorderOutline);
...
if ( $Filtered ) {
	$objPHPExcel->getActiveSheet()->setCellValue('L6', 'Filtered');
	$objPHPExcel->getActiveSheet()->getStyle('L6')->applyFromArray($styleMediumBlackBorderOutline);
}
...
if ( $c > 0 ) {
	...
	while ( $row = mysql_fetch_array($result)) {
		...
		if ( substr($row['IngredientProductNumber'], 0, 1) == 4 ) {
			...
			$objPHPExcel->getActiveSheet()->mergeCells('B'.$i.':L'.$i);
			$objPHPExcel->getActiveSheet()->getStyle('B'.$i.':L'.$i)->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			$objPHPExcel->getActiveSheet()->getStyle('B'.$i.':L'.$i)->getFill()->getStartColor()->setARGB('FFFFFF99');
			$objPHPExcel->getActiveSheet()->getStyle('B'.$i.':L'.$i)->applyFromArray($styleMediumBlackBorderOutline);
			continue;
			...
			while ( $row_vend = mysql_fetch_array($result_vend) ) {
				...
				if ( $c > 1 ) {
				  ...
				  if ( $current_id == "" ) 
					...
					if ( $Filtered ) {
						$objPHPExcel->getActiveSheet()->getStyle('L'.$i)->applyFromArray($styleMediumBlackBorderOutline);
					}
				} 
...
$objPHPExcel->getActiveSheet()->setCellValue('D'.$i, 'Filtered:  '.$Filtered);
$objPHPExcel->getActiveSheet()->mergeCells('D'.$i.':E'.$i);	
$objPHPExcel->getActiveSheet()->setCellValue('F'.$i, 'Approved By:');
$objPHPExcel->getActiveSheet()->mergeCells('F'.$i.':G'.$i);	
$objPHPExcel->getActiveSheet()->setCellValue('H'.$i, $ApprovedBy);
$objPHPExcel->getActiveSheet()->mergeCells('H'.$i.':K'.$i);	
