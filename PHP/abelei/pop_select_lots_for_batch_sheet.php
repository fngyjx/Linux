...
//die();
	...
	if ( $c > 0 ) {
		...
		while ( $row = mysql_fetch_array($result) ) {
			if ( '108290' == $row[IngredientProductNumber] ) // Ignore Water
				continue;
			...
			while ( $row = mysql_fetch_array($result) ) { 
				...
				foreach (array_keys($_POST) as $key) {
					...
					if ( substr($key, 4, 6) == $row[PackIn] ) {
						$c++;
						$lot = $_POST["lot_$row[PackIn]_$row[CustomerOrderNumber]_$c"];
						$qty = prep_number($_POST["qty_$row[PackIn]_$row[CustomerOrderNumber]_$c"]);
...
// print_r($_POST); echo "</h3>";
		if ( $c > 0 ) {
			...
			$run_deletes = false;
			while ( $row = mysql_fetch_array($result) ) {
				if ( '108290' != $row[IngredientProductNumber] and 4 != substr($row[IngredientProductNumber], 0, 1) and 0 < $row[Quantity] ) {
					$bg = 0;
					$i++;
					$c = 0;
					foreach (array_keys($_POST) as $key) { 
...
// echo $sql . "<BR>";
				...
				} // else echo "<h2>Boom 1( '108290' == $row[IngredientProductNumber] or 4 == substr($row[IngredientProductNumber], 0, 1) or 0 >= $row[Quantity] ) </h2>";  // SHOW INGREDIENT IF NOT INSTRUCTIONS OR WATER
			}   // WHILE LOOPING THROUGH INGREDIENTS
		// And then loop through packaging.
		$sql = "SELECT bsci.*, pm.Designation FROM batchsheetcustomerinfo AS bsci, productmaster AS pm ".
			"WHERE pm.ProductNumberInternal=bsci.PackIn AND BatchSheetNumber = $bsn";
		$result = mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
		$c = mysql_num_rows($result);
		$bg = 0; 
		if ( $c > 0 ) {
						...
						if ( $lot != '' and $qty != '' ) {
								$run_deletes = true;
									$sql = "INSERT INTO inventorymovements ".
										...
										"VALUES ".
										"(" . escape_data($lot) . ", " . escape_data($row[PackIn]) . ", " . escape_data($qty) . ", 8, 'C', '" . date("Y-m-d H:i:s") . "', 'Cust PO# " . escape_data($row[CustomerPONumber]) . " - " . escape_data($row[CustomerCodeNumber]) . "')";
									mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
									$trans_num = mysql_insert_id();
// echo $sql . "<BR>";
									...
									$sql = "INSERT INTO batchsheetdetailpackaginglotnumbers ".
										...
										"VALUES ".
										"(" . escape_data($bsn) . ", '" . escape_data($row[CustomerOrderNumber]) . "', '" . escape_data($row[CustomerOrderSeqNumber]) . "', '" . escape_data($row[CustomerPONumber]) . "', '" . escape_data($row[PackIn]) . "', " . escape_data($lot) . ", " . escape_data($trans_num) . ", " . escape_data($qty) . ")";
									mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
// echo $sql . "<BR>";
...
$sql = "SELECT bsd.*, pm.Designation, pm.UnitOfMeasure, Quantity 
...
if ( $c > 0 ) {
	...
	while ( $row = mysql_fetch_array($result) ) {
		...
		if ( '108290' == $row[IngredientProductNumber] or 4 == substr($row[IngredientProductNumber], 0, 1))
			continue;
		if ( 0 >= $row[Quantity] ) {
				...
				$c = 0;
					...
					<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="2">
						...
						<?php $bg=0; while ( $row_lots = mysql_fetch_array($result_lots) ) {
							...
							$c++;
							if ( isset($_POST["qty_" . $row['IngredientProductNumber'] . "_" . $c]) ) 
							{
								$subQuantity = prep_number($_POST["qty_" . $row['IngredientProductNumber'] . "_" . $c]);
							}
							else
							{
								if ($quantity_needed > $row_lots['InventoryCount']) 
							...
							<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
								...
								<TD ALIGN=RIGHT><?php echo $row_lots['LotSequenceNumber'];?></TD>
								<TD><IMG SRC="images/spacer.gif" WIDTH="3" HEIGHT="1"></TD>
								<TD ALIGN=RIGHT><?php echo number_format($row_lots['InventoryCount']);?></TD>
								<TD><IMG SRC="images/spacer.gif" WIDTH="3" HEIGHT="1"></TD>
								<TD>
								<INPUT TYPE="hidden" NAME="<?php echo "lot_$row[PackIn]_$row[CustomerOrderNumber]_$c";?>" VALUE="<?php echo $row_lots[ID];?>" SIZE="8" STYLE="text-align:right">
								<INPUT TYPE="text" NAME="<?php echo "qty_$row[PackIn]_$row[CustomerOrderNumber]_$c";?>" VALUE="<?php echo number_format($subQuantity,2) ?>" SIZE="15" STYLE="text-align:right">
								</TD>
							</TR>
						<?php } ?>
				</TABLE>
