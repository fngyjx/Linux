...
if ( $_GET['action'] == "inact" ) {
	...
	mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
	header("location: vendors_vendors.php?alpha=".$_GET['alpha']);
...
<?php if ( $_GET['action'] == "" ) { ?>
	...
	if ( $_GET['alpha'] ) {
		...
		<?php
		$alpha = $_GET['alpha'];
		$sql = "SELECT DISTINCT vendor_id, name FROM vendors WHERE name LIKE '$alpha%' AND active = 1 ORDER BY name";
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="1">
				...
				while ( $row = mysql_fetch_array($result) ) {
					...
					<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
						...
						<?php } ?>
							<INPUT TYPE="button" VALUE="Delete" onClick="inactivate('<?php echo $row['vendor_id'].'\',\''.$row['name'].'\',\''.$alpha;?>')" STYLE="font-size:7pt">
						</TD>
...
function inactivate(vid,name,alpha) {
	if ( confirm('Are you sure you want to remove vendor ' + name +'?') ) {
		document.location.href = "vendors_vendors.php?action=inact&vid=" + vid+"&alpha="+alpha;
	}
}
