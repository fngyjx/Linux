...
function validate() {
	global $po_id, $lot_number, $lot_sequence, $employee_id, $invoice_number, $date_received, $date_received_submit, $package_type, $product_number, $vendor_id, $vendor_product_code, $manufacture_date, $manufacture_date_submit, $expiration_date, $expiration_date_submit, $quantity, $pack_size, $measurement_units, $allergen_egg, $allergen_milk, $allergen_peanut, $allergen_seafood, $allergen_seed, $allergen_soybean, $allergen_sulfites, $allergen_tree_nuts, $allergen_wheat, $allergen_yellow, $package_type, $shipment_condition, $storage_location, $has_c_of_a, $nutrition_on_file, $msds_on_file, $allergen_on_file, $specifications_on_file, $kosher_approved,$qc_date, $qc_date_submit, $qc_employee_id, $retain_size, $comments, $status, $link, $record_id;
...
/*	if ( isset($_REQUEST['qc_day']) ) { $qc_day=escape_data($_REQUEST['qc_day']); }
	...
	if ( "" == $lot_number ) { 
		...
		$lot_good=true;
		if (10 < strlen($lot_number) ) { $error_message .= "Lot Number cannot be longer than 10 characters.<br/>"; $lot_good=false; }
	...
	if ("" == $error_message) {
		...
		switch ($status) {
			...
			case "A": //status "Approved"
				...
				mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
				$sql  = "UPDATE lots, receipts SET lots.StorageLocation='$storage_location' WHERE receipts.LotID=lots.ID AND receipts.ID='$record_id'";
				mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
				break;
			case "P": //status "Pending"
				...
				$sql  = "SELECT QuantityConvert( Quantity, 'grams', '$measurement_units' ) AS Quantity, ProductNumberInternal FROM inventorymovements WHERE TransactionNumber = ( ";
				$sql .= "SELECT InventoryMovementTransactionNumber FROM lots WHERE ID = ( ";
 ...
 if ( "search" != $action && "" != $action ) {
	if ("new" != $action) {
		...
		if (""!=$record_id ) {
			...
			$sql .= "productmaster.AllergenSeed, productmaster.AllergenSoybean, productmaster.AllergenSulfites, productmaster.AllergenTreeNuts, productmaster.AllergenWheat, productmaster.AllergenYellow, ";
...
<?php } ?>
			...
			<TR>
				...
				<TD>
					...
					Sequence #: <?php echo formatTxt(getFormSafe($lot_sequence)); ?>
				</TD>
			</TR>
			<TR>
		...
		<tr><td align="center" style="padding-left:1em;">
			...
			<TR>
				<TD align="right">Size of Retain Taken: </TD><TD align="left"><INPUT TYPE="text" NAME="retain_size"  VALUE="<?php echo formatTxt(getFormSafe($retain_size)) ?>" <?php echo ($readonly_retain_size ? "READONLY ": "") ?>SIZE="30" /></TD>
			</TR>
			</table>
		</tr></td>
			</table>
