...
if ( $action == 'inact' ) {
	//delete the phone/fax number 
	if ( !empty($pid) ) {   
		$sql = "DELETE FROM vendor_address_phones where phone_id='". $pid . "'";
		mysql_query($sql,$link) or die(mysql_error() . " Failed Execute : $sql <br />");
	}
}

if ( !empty($_POST) and empty($_REQUEST['number']) ) {
	...
	if ( $company_phone != "" )
		check_field($company_phone, 1, 'Company Phone');
	if ( $company_fax != "" )
	check_field($company_fax, 1, 'Company fax');
	...
	if ( !$error_found ) {
		...
		$company_phone = escape_data($company_phone);
		$company_fax = escape_data($company_fax);
		...
		if ( $aid != "" ) {
			...
			if ( strlen($company_phone) >= 10 ) { 
				$sql = "UPDATE vendor_address_phones
				SET number = '$company_phone'
				where address_id = $aid AND type=2 AND number_description='MainPhone'";
				mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
			}
			if ( strlen($company_fax) >= 10 ) { 
				$sql = "UPDATE vendor_address_phones
				SET number = '$company_fax'
				where address_id = $aid AND type=4 AND number_description='MainFax'";
				mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
			}
		}
		else {
			...
			if ( strlen($company_phone) >= 10 ) { 
				$sql = "INSERT INTO vendor_address_phones (address_id, number, type, number_description)
				VALUES('".$insert_id."','".$company_phone."','2','MainPhone')";
			}
			if ( strlen($company_fax) >= 10 ) { 
				$sql = "INSERT INTO vendor_address_phones (address_id, number, type, number_description)
				VALUES('".$insert_id."','".$company_fax."','4','MainFax')";
			}
		}
...
} else if ( !empty($_REQUEST['subform']) )  { 

	$number_description = escape_data($_REQUEST['number_description']);
	$phone_number= escape_data($_REQUEST['number']);
	$phone_type =  escape_data($_REQUEST['type']);
	$sql= "INSERT INTO vendor_address_phones ( address_id,number,type,number_description )
		VALUES('$aid','$phone_number','$phone_type','$number_description')";
...
if ( $aid != '' ) {
		...
		$zip = $row[zip];
		$country = $row[country];
		$company_phone = $row['number'];
		$notes = $row[notes];
		$ship_to_vendor = $row[ship_to_vendor];
		$sql = "SELECT if(type=2,'phone','fax'),number FROM vendor_address_phones WHERE address_id=$aid AND type in(2,4)";
		$result = mysql_query($sql,$link) or die ( mysql_error() . " Failed Execute SQL : $sql <br />");
		while ( $row = mysql_fetch_array($result) ) {
			if ( $row[0] == 'phone' )
				$company_phone = $row['number'];
			if ( $row[0] == 'fax' )
				$company_fax = $row['number'];
		}	
}
...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
	...
	<TR>
		<TD><B CLASS="black">Company Phone#:</B></TD>
		<TD><INPUT TYPE='text' NAME="company_phone" SIZE=26 VALUE="<?php echo stripslashes($company_phone);?>"></TD>
	</TR>
	<TR><TD COLSPAN=2><IMG SRC="images/spacer.gif" WIDTH="1" HEIGHT="9"></TD></TR>
	<TR>
		<TD><B CLASS="black">Company Fax#:</B></TD>
		<TD><INPUT TYPE='text' NAME="company_fax" SIZE=26 VALUE="<?php echo stripslashes($company_fax);?>"></TD>
	</TR>
	<TR><TD COLSPAN=2><IMG SRC="images/spacer.gif" WIDTH="1" HEIGHT="5"></TD></TR>
	...
	<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<TR>
			...
			<SELECT NAME="type" STYLE="font-size:7pt">
				<?php
				$sql = "SELECT type_id, if ( type_id=2,'Phone', 'Fax') as description FROM phone_types where type_id in (2,4) ORDER BY type_id";
				$result = mysql_query($sql, $link);
	...
	<?php
	$sql = "SELECT phone_id, number, if ( type=2,'Phone','Fax') as description, number_description FROM vendor_address_phones LEFT JOIN phone_types ON vendor_address_phones.type = phone_types.type_id WHERE vendor_address_phones.type in (2,4) AND address_id = " . $aid . " ORDER BY type_id";
	$result = mysql_query($sql, $link);
	if ( mysql_num_rows($result) > 0 ) {
		$bg = 0; ?>
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			if ( $bg == 1 ) {
				...
				<TR BGCOLOR="<?php echo($bgcolor);?>" VALIGN=TOP>
					<TD><A HREF="pop_add_vendor_address.php?vid=<?php echo $vid;?>&pid=<?php echo $row['phone_id'];?>&aid=<?php echo $aid;?>"><IMG SRC="images/pencil.gif" WIDTH="16" HEIGHT="16" BORDER=0></A></TD>
...
function inactivate(pid, vid, aid) {
	if ( confirm('Are you sure you want to delete this number?') ) {
		document.location.href = "pop_add_vendor_address.php?action=inact&pid=" + pid + "&vid=" + vid + "&aid=" + aid
	}
}
