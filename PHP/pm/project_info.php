...
if ( $_GET['stat'] ) {
	...
	if ( $_GET['stat'] == "2" ) {   // STATUS: FROM SALES TO LAB
		...
		//send email to lab people and Karen, Troy
		include('auto_email.php');

		$sql = "SELECT projects.*, concat(clients.first_name,' ',clients.last_name) as Client, company 
		FROM projects LEFT JOIN clients USING(client_id) LEFT JOIN companies USING (company_id)
		WHERE project_id='".$_SESSION['pid']."'";

		$result=mysql_query($sql,$link) or die ( mysql_error() . " Failed Execute SQL:$sql<br />");
		$row=mysql_fetch_array($result);
		
		$sql = "SELECT email from users where ( user_type=3 OR (user_id=".$row['salesperson']." AND user_id <> 10 ) ) AND active=1";
		$result_user=mysql_query($sql,$link) or die ( mysql_error() . " Failed to Execute SQL: $sql<br />");
		$row_user=mysql_fetch_array($result_user); 
		$email=$row_user[0];
		while ( $row_user=mysql_fetch_array($result_user) ) {
			$email.=",".$row_user[0];
		}
		$project_id=substr($row['project_id'], 0, 2) . "-" . substr($row['project_id'], -3);
		$subject=$project_id.",".$row['summary']."(".$row['company'].") Due On: ".date("n/d/Y",strtotime($row['due_date']));
		$message="<b>New lab project</b><BR><BR>";
		$message.="<b>Project#:</b> ".$project_id.";&nbsp;&nbsp;&nbsp;  <b>Priority:</b> ".$priority_array[$row['priority']-1]."; ;&nbsp;&nbsp;&nbsp; <b>Client:</b> ".$row['Client']."/".$row['company'];
		$message.="<BR><b>Type:</b> ".$project_type_array[$row['project_type']-1]."; &nbsp;&nbsp;&nbsp;<b>Status:</b>".$status_array[$row['status']-1].";&nbsp;&nbsp;&nbsp; <b>Due Date:</b> ". date("m/d/Y",strtotime($row['due_date']));
		$message.="<br /><br /><b>N_or_A:</b> ".$n_a1_array[$row['n_a1']-1].( $row['n_a2'] == "" ? "" : ", ".$n_a2_array[$row['n_a2']-1]); 
		$message.="; &nbsp;&nbsp;&nbsp;<b>Form:</b> ".$form_array[$row['form']-1]. ( $row['form_2'] == "" ? "" : ", ".$form_array[$row['form_2']-1]);
		$message.=";&nbsp;&nbsp;&nbsp; <b>Product Type:</b> ".$product_type_array[$row['product_type']-1]. ( $row['product_type_2'] == "" ? "" : ", ".$product_type_array[$row['product_type_2']-1])."<BR>";
		$message.="<b>Kosher:</b> ". $kosher_array[$row['kosher']-1].";&nbsp;&nbsp;&nbsp; <b>Halal:</b> ".$halal_array[$row['halal']-1];
		$message.=";&nbsp;&nbsp;&nbsp;<b>Sample Size</b>". ($row['sample_size'] == 5 ? $row['sample_size_other'] : $sample_size_array[$row['sample_size']-1])."<br />";
		$message.="<b>Base Included:</b> " . $base_included_array[$row['base_included']-1] .";&nbsp;&nbsp;&nbsp;<b>Target Included:</b> ". $target_included_array[$row['target_included']-1];
		$message.="<br />---------------------------<br />";
		$message.="<b>Comments:</b><br />---------------------------<br />";
		$message.=str_replace($text_newline,$html_newline,$row['comments']);
		$cc="kcriss@abelei.com,tgooding@abelei.com";
		$from=$_SESSION['uNameCookie'];
		$bcc="jdu@abelei.com";
		$email.= ($from == "tgooding@abelei.com" ? "" : ",".$from);
	...
	elseif ( $_GET['stat'] == "3" ) {   // STATUS: FROM LAB TO FRONT DESK
		...
		$sql = "SELECT company, sample_size, sample_size_other, users.email
		FROM projects
		LEFT JOIN clients
		USING ( client_id ) 
		LEFT JOIN companies
		...
		$to = $email;   //"$email";
		$cc = "cpeters@abelei.com";   
		$bcc = "shenderson@abelei.com";
		...
		$mime->addCc("$cc");
		$mime->addBcc("$bcc");
		$mime->setSubject($_SESSION['pid'] . " - " . $company . ", Shipping Decision Needed");
...
this message. <BR><BR>";
		...
		$bcc = "tgooding@abelei.com,shenderson@abelei.com";   // "tgooding@abelei.com,$from,
...
if ( isset($_SESSION['pid']) ) {
	...
	if ( !empty($_POST) ) {
		...
		if ( !$error_found ) {
			...
			$sql = "SELECT due_date FROM projects WHERE project_info_submitted = 1 AND due_date <> '" . $due_date . "' AND project_id = " . $_SESSION['pid'];
			$result = mysql_query($sql, $link);
...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
	...
	<TR>
		...
		if ( $_SESSION['userTypeCookie'] == 1 and $status < 4 ) {
			...
			$sql = "SELECT user_id, first_name, last_name FROM users WHERE user_type = 2 OR last_name = 'Gooding' OR user_id = 11 ORDER BY user_type, last_name";
			$result = mysql_query($sql, $link);
			$c = mysql_num_rows($result);
			if ( $c != 0 ) { ?>
