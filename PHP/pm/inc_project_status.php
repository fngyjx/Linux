...
if ( $status == 1 ) {
	...
	if ( $row['project_info_submitted'] == 1 and $row['client_info_submitted'] == 1 and $row['sample_info_submitted'] == 1 ) { ?>
		<FORM><INPUT TYPE="button" VALUE="Send to lab" onClick="window.location='project_info.php?stat=2'"></FORM>
	<?php }
	else { ?>
		<FORM><INPUT TYPE="button" VALUE="Send to lab" DISABLED> <B>Incomplete:</B>
		<?php if ( $row['project_info_submitted'] == 0 ) { ?>
			<I>&bull;Sales Info</I>
		<?php } ?>
		<?php if ( $row['client_info_submitted'] == 0 ) { ?>
			<I>&bull;Client Info</I>
		<?php } ?>
...
if ( $_SESSION['userTypeCookie'] == 3 ) {
	...
	if ( $status == 2 ) {
		//if ( $i > 0 ) { ?>
			<INPUT TYPE="button" VALUE="Send to front desk" onClick="window.location='project_info.php?stat=3'">
		<?php //}
		//elseif ( $i == 0 ) { ?>
			<!-- <INPUT TYPE="button" VALUE="Send to front desk" DISABLED>  <B>Incomplete:</B> -->
