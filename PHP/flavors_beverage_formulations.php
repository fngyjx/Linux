...
if ( $edit and isset($_POST['add_ing']) ) {
	...
	$tmpArr= explode("&nbsp;",$_POST['IngredientProductNumber']);
	$IngredientProductNumber = $tmpArr[0];
...
$(document).ready(function(){
	...
	$(":submit").click(function() {
		...
		{
			case 'new':
				popup('pop_add_beverage.php',800,900);
				return false;
				break;
			default:
...
/*	$("#clone_formula").click(function() {
		document.location.href = "flavor_formula_clone.php?pne=<?php echo $pne ?>";
	});
	*/
	...
	$("#IngredientProductNumber").autocomplete("search/product_master_formulas_by_internal_number.php", {
		cacheLength: 1,
		width: 650,
		max:50,
    scroll: true,
	...
	$("#IngredientProductNumber").result(function(event, data, formatted) {
		if (data){
		  var datastr = String(data);
		  var rcrdindx = datastr.indexOf('&nbsp;');
		  datastr=datastr.substr(0,rcrdindx);
		  $("#IngredientProductNumber").val(datastr);
					}
	});
});
...
<tr valign="top">
<td class="padded">
	<FORM id="search" name="search" ACTION="flavors_beverage_formulations.php" METHOD="get">
	<INPUT TYPE="hidden" NAME="action" VALUE="search">

	<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">

		<TR>
			<TD><B>Beverage designation:</B></TD>
		...
		<TR>
			<TD colspan=3 ALIGN=left>
				<INPUT style="float:right" name="search" id="search" TYPE="submit" class="submit_medium" VALUE="Search"><INPUT style="margin-top:.5em" name="new" id="new" TYPE="submit" class="submit new" VALUE="New Beverage">
			</TD>
		</TR>
	</TABLE>
</FORM>
...
if ( $action == 'search' ) {
	...
	$sql .= "FROM externalproductnumberreference RIGHT OUTER JOIN productmaster ON externalproductnumberreference.ProductNumberInternal = productmaster.ProductNumberInternal ";
...
if ( $action == "edit" ) {
	...
	if ( $_REQUEST['update'] != 1 and $update != 1 ) {
		...
		$display_top = "style=\"display:none\"";
		$hide_top = "";
	} elseif ($_RESQUEST['update'] == 1 or $_GET['update'] == 1 ) {
		$hide_top = "style=\"display:none\"";
		$display_top = "";
	}

	?>
	
	<div name="hide_verbs" id="hide_verbs" <?php echo $display_top; ?>>Check This <input type="checkbox" name="hide_topps" id="hide_topps" <?php echo $display_top; ?>> to Hide Following Table </div>
	<div name="show_verbs" id="show_verbs" <?php echo $hide_top; ?>>Check This <input type="checkbox" name="show_topps" id="show_topps" <?php echo $hide_top; ?>> to Show Formular Table </div>
	
    <div id="site_top" name="site_top" <?php echo $display_top; ?>>
	...
	<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<TR VALIGN=TOP>
			...
			<?php if ( $form_status != '' ) { ?>
				<?php echo $QuickScan;?>
			<?PHP } else { ?>
				<INPUT TYPE="text" NAME="QuickScan" VALUE="<?php echo $QuickScan;?>" SIZE="55" MAXLENGTH="45" STYLE="width: 310px">
			<?php } ?>
			</TD>
		</TR>
...
*/?>				</TR>
		...
		<TR>
			...
			while ( $row_dev = mysql_fetch_array($result_dev) ) {
				...
				} elseif ( $row_dev['user_id'] == 2 ) { //Jennifer as default
					echo "<OPTION VALUE='" . $row_dev['user_id'] . "' SELECTED>" . $row_dev['first_name'] . ' ' . $row_dev['last_name'] . "</OPTION>";
				} else {
	...
	<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
		...
		<TR VALIGN=TOP>
			...
			<TD WIDTH=350>
			<?php if ( $form_status != '' ) { ?>
				<?php echo str_replace($text_newline,$html_newline,$NoteForFormulation);?>
			<?php } else { ?>
		...
		<TR><TD COLSPAN=3 height="40"><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="1" HEIGHT="5"></TD></TR>
...
<!-- 
 ...
 -->
		<FORM NAME="add_ingredient" id="add_ingredient" ACTION="flavors_formulations.php" METHOD="post">
		<INPUT TYPE="hidden" NAME="action" VALUE="<?php echo $action;?>">
		<INPUT TYPE="hidden" NAME="ProductNumberInternal" VALUE="<?php echo $ProductNumberInternal;?>">
		<INPUT TYPE="hidden" NAME="pne" VALUE="<?php echo $pne;?>">
			...
			<TR>
				...
				<TABLE ALIGN=RIGHT BORDER=0 CELLSPACING="0" CELLPADDING="2" BORDERCOLOR="#CDCDCD">
					...
					<TR>
						...
						<TD><INPUT TYPE="text" id="IngredientProductNumber" NAME="IngredientProductNumber" VALUE="<?php echo $IngredientProductNumber;?>" SIZE="12" ></TD>
	...
	<TABLE BORDER="0" WIDTH="100%" CELLSPACING="0" CELLPADDING="0">
		<TR>
			<TD><B>Formula</B></TD>
			<td>
				Abelei#:<?php echo $ProductNumberExternal ;?> Internal#:<?php echo $ProductNumberInternal;?>
			</td>
		</TR>
	</TABLE>
	...
	if ( $c > 0 ) {
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			<INPUT TYPE="hidden" NAME="update_ing" VALUE="<?php echo $row['IngredientProductNumber'];?>">
				...
				if ( substr($row['IngredientProductNumber'], 0, 1) == 4 ) {
					...
					$water_colspan = "";
					$colspan = "COLSPAN=10";
				} elseif ( substr($row['IngredientProductNumber'],0,5) == "10829" ) {  // WATER
					$td_bgcolor = "#F3E7FD";
					$font_color = "";
				...
				?>
					...
					} elseif ( substr($row['IngredientProductNumber'],0,5) == "10829" ) {
						$vendor_name = "&nbsp;";
					} else {
					...
					<?php  if ( substr($row['IngredientProductNumber'],0,5) != "10829" ) { ?>
						...
						if ( is_numeric($vendor_price_per_pound) ) {
							...
							echo number_format($selected_line_item*0.01, 4);
							$TotalSelectedPricePerPound += $selected_line_item;
						} else {
							echo "&nbsp;";
						}
						?>
						...
						if ( is_numeric($row['MaxEstimatedPricePerPound']) ) {
							...
							echo number_format($max_line*0.01, 2);
							$totalMax += $max_line;
						} else {
							echo "&nbsp;";
						}
		...
		<TR>
			...
			<TD ALIGN=RIGHT><B><?php echo number_format($TotalSelectedPricePerPound*0.01, 2);?></B></TD>
 

			<TD>&nbsp;</TD>

			<TD ALIGN=RIGHT><B><?php echo number_format($TotalLeastEstimatedPricePerPound*0.01, 2);?></B></TD>
			<TD ALIGN=RIGHT><B><?php echo number_format($totalMax*0.01, 2);?></B></TD>
			<TD>&nbsp;</TD>
			<TD>&nbsp;</TD>
...
<!-- 
 ...
 -->
	...
	<?php if ( "" != $form_status and $_REQUEST['update_ing'] == '' ) { ?>
		...
		<INPUT TYPE="button" VALUE="Clone Formula" CLASS="submit" id="clone_formula" STYLE="margin-right:10px; margin-left:10px;" onClick="popup('flavor_formula_clone.php?pne=<?php echo $pne; ?>&ProductNumberInternal=<?php echo $ProductNumberInternal; ?>')"> 
