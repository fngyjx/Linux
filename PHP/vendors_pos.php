...
if ( isset($_POST['edit_prod']) ) {
	...
	if ( !$error_found ) {
		...
		if ( $VendorID != "" ) {
			...
			$sql_vend = "SELECT * from vendors 
				LEFT JOIN vendor_addresses USING(vendor_id)
				WHERE vendors.vendor_id='".$VendorID."'";
			$result_vend = mysql_query($sql_vend,$link) or die ( mysql_error() . " Failed Execute SQL : $sql_vend <br />");
			$row_vend=mysql_fetch_array($result_vend);
		
			$sql_upd = "UPDATE purchaseordermaster set VendorID='".$VendorID."', VendorName='".$row_vend['name']."'".
			", VendorStreetAddress1='".$row_vend['address1']."',".
			" VendorStreetAddress2='".$row_vend['address2']."',".
			" VendorCity='".$row_vend['city']."',".
			" VendorState='".$row_vend['state']."',".
			" VendorZipCode='".$row_vend['zip']."'".
			" WHERE PurchaseOrderNumber='".$pon."'";
			mysql_query($sql_upd,$link) or die (mysql_error() . " FAILED execute SQL : $sql_upd <br />");
		} 
...
//	check_field($ConfirmationOrderNumber, 1, 'Order#');
	...
	if ( !$error_found ) {
		...
		if ( $pon != '' ) {
			...
			mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
			//echo "<br /> $sql <br />";
			$sql = "UPDATE purchaseorderdetail set PurchaseOrderNumber=$PurchaseOrderNumber WHERE PurchaseOrderNumber=$pon";
			mysql_query($sql, $link) or die ( mysql_error() ." Failed execute SQL : $sql <br />");
			
			//echo "<br /> $sql <br />";
			$pon = $PurchaseOrderNumber;
		} else {
			$sql = "INSERT INTO purchaseordermaster (PurchaseOrderNumber, PurchaseOrderType, VendorID, 
			VendorName, VendorStreetAddress1, VendorStreetAddress2, VendorCity, VendorState, VendorZipCode,
			VendorMainPhoneNumber, ShipToID, ShipToName, ShipToStreetAddress1, ShipToStreetAddress2, ShipToCity,
			ShipToState, ShipToZipCode, ShipToMainPhoneNumber, ShippingAndHandlingCost, PaymentType, ShippingDate,
			DateOrderPlaced, ConfirmationOrderNumber, contact_id, VendorSalesRep, ShipVia, Notes) 
			VALUES ('" . $PurchaseOrderNumber . "', '" . $PurchaseOrderType . "', '" . $VendorID . "', '" 
			. $VendorName . "', '" . $VendorStreetAddress1 . "', '" . $VendorStreetAddress2 . "', '"
			. $VendorCity . "', '" . $VendorState . "', '" . $VendorZipCode . "', '"
			. $VendorMainPhoneNumber . "', '" . $ShipToID . "', '" . $ShipToName . "', '"
			. $ShipToStreetAddress1 . "', '" . $ShipToStreetAddress2 . "', '" . $ShipToCity . "', '" 
			. $ShipToState . "', '" . $ShipToZipCode . "', '" . $ShipToMainPhoneNumber . "', '" 
			. $ShippingAndHandlingCost . "', '" . $PaymentType . "', " . $NewShippingDate . ", '" 
			. date("Y-m-d H:i:s") . "', '" . $ConfirmationOrderNumber . "', " . $contact_id_clause . ", '" 
			. $VendorSalesRep . "', '" . $ShipVia . "', '" . $Notes . "')";
			mysql_query($sql, $link) or die (mysql_error()."<br />Couldn't execute query: $sql<BR><BR>");
			$pon = $PurchaseOrderNumber;
		}
	...
	if ( $row['VendorID'] == "" and $_REQUEST['VendorID'] != "" ) {
		$VendorID = escape_data($_REQUEST['VendorID']);
		$sql_vend = "SELECT * from vendors 
		LEFT JOIN vendor_addresses USING(vendor_id)
		WHERE vendors.vendor_id='".$VendorID."'";
		$result_vend = mysql_query($sql_vend,$link) or die ( mysql_error() . " Failed Execute SQL : $sql_vend <br />");
	...
	$ShipToID = ( empty($row['ShipToID']) ) ? "1" : $row['ShipToID'];
	$ShipToName = ( empty($row['ShipToName']) ) ? "albelei" : $row['ShipToName'];
	$ShipToStreetAddress1 = ( empty($row['ShipToStreetAddress1']) ) ? "194 Alder Dr." : $row['ShipToStreetAddress1'];
	$ShipToStreetAddress2 = $row['ShipToStreetAddress2'];
	$ShipToCity = ( empty($row['ShipToCity']) ) ? "North Aurora" : $row['ShipToCity'];
	$ShipToState = ( empty($row['ShipToState']) ) ? "IL" : $row['ShipToState'];
	$ShipToZipCode = ( empty($row['ShipToZipCode']) ) ? "60542" : $row['ShipToZipCode'];
	$ShipToMainPhoneNumber = ( empty($row['ShipToMainPhoneNumber']) ) ? "630-859-1410" : $row['ShipToMainPhoneNumber'];
	...
	$sql="SELECT * FROM purchaseorderdetail WHERE PurchaseOrderNumber = $pon";
	$result=mysql_query($sql,$link) or die (mysql_error() . " Failed execute SQL: $sql <br />");
	$c=mysql_num_rows($result);
	
	$status = ($c > 0 )? $row['incompletes'] : 1;
	
	if ( $PuchaseOrderNumber != $pon and $PuchaseOrderNumber != "")  {
		$sql = "UPDATE puchaseordermaster set PurchaseOrderNumber=$PurchaseOrderNumber where PurchaseOrderNumber=$pon";
		mysql_query($sql,$link) or die ( mysql_error() . " Fialed execute SQL: $sql <br />");
		
		$sql = "UPDATE puchaseorderdetail set PurchaseOrderNumber=$PurchaseOrderNumber where PurchaseOrderNumber=$pon";
		mysql_query($sql,$link) or die ( mysql_error() . " Fialed execute SQL: $sql <br />");
		$pon=$PuchaseOrderNumber ;
	}
...
<td class="padded">
	...
	<INPUT TYPE="hidden" NAME="action" VALUE="search">
		...
		<TR>
			<TD><B>Vendor:</B></TD>
			<TD><IMG SRC="images/spacer.gif" ALT="spacer" WIDTH="10" HEIGHT="1"></TD>
			<TD><INPUT TYPE="text" NAME="vendor" id="vendor" SIZE="20" VALUE="<?php echo stripslashes($vendor);?>"></TD>
		</TR>
...
<BR><BR>
	...
	if ( $_REQUEST['action'] != 'search' ) {
		...
		$sql = "SELECT DISTINCT purchaseordermaster.PurchaseOrderNumber, VendorID, VendorName, DateOrderPlaced,
			PurchaseOrderType, count(purchaseorderdetail.ID) as li_count,purchaseorderdetail.Status as Status
        FROM purchaseordermaster
		LEFT JOIN purchaseorderdetail USING (PurchaseOrderNumber)
        WHERE (purchaseorderdetail.ProductNumberInternal not like '7%' or purchaseorderdetail.ProductNumberInternal is null)
		AND ( purchaseordermaster.PurchaseOrderType <> 'Process' Or purchaseordermaster.PurchaseOrderType is null)
		AND (purchaseorderdetail.Status in ('O', 'P','') OR purchaseorderdetail.Status IS NULL  
        OR  NOT EXISTS (SELECT PurchaseOrderNumber FROM purchaseorderdetail where PurchaseOrderNumber=purchaseordermaster.PurchaseOrderNumber))
	    GROUP BY PurchaseOrderNumber, Status
	...
	//	echo $sql . "<BR>";
		...
		<h3>All open POs</h3>
			...
			while ( $row = mysql_fetch_array($result) ) {
				...
				<TD>
				<?php
				$sql = "SELECT receipts.ID, receipts.PurchaseOrderID, receipts.Status as r_status, purchaseorderdetail.Status as p_status
				FROM receipts
					LEFT JOIN purchaseorderdetail ON receipts.PurchaseOrderID = purchaseorderdetail.ID
				...
				<?php } else {
					while($row_r=mysql_fetch_array($result_delete) ) {
						if ( $row_r['r_status'] != $row_r['p_status'] ) {
							$sql="UPDATE purchaseorderdetail set Status='".$row_r['r_status']."' ".
							" WHERE PurchaseOrderNumber='$row[PurchaseOrderNumber]' 
							AND ID='$row_r[PurchaseOrderID]'";
							
							//echo "<br /> $sql <br />";
							mysql_query($sql,$link) or die (mysql_error() ." <br />Failed to execute SQL: $sql <br />");
						}
					}
				}
				?>
				...
				<!-- <TD><A HREF="JavaScript:void(0)" onClick="printer_popup('reports/print_formula.php?pni=<?php echo $row['ProductNumberInternal'];?>')"><?php //echo $row['ProductNumberExternal'];?></A></TD> -->
					...
					?></TD>
						...
						else if ( $row['Status'] == "" or $row['Status'] == 'O' and $k == 0 ) {
								$Status="Open";
								$color="yellow";
						} 
						else if ( $row['Status'] == 'P' and $k == 0 ) {
							$Status="Ordered";
							$color="#FFFF99";
						} else {
							$Status="Approved";
...
if ( $_REQUEST['action'] == 'search' ) {
	...
	if ( $Status == 'O' ) {
		$Status_clause = " AND ( Status = 'O' ) ";

	} elseif ( $Status == 'A' ) {
		$Status_clause = " AND ( Status = 'A' ) ";
	} elseif ( $Status == 'P' ) {
		$Status_clause = " AND ( Status = 'P' ) ";
	} else {
		$Status_clause = "";
	}
	...
	$sql = "SELECT DISTINCT purchaseordermaster.PurchaseOrderNumber, VendorID, VendorName, DateOrderPlaced, 
		PurchaseOrderType, COUNT(purchaseorderdetail.ID) AS li_count, 
	purchaseorderdetail.Status AS Status
	FROM purchaseordermaster
	LEFT JOIN purchaseorderdetail USING (PurchaseOrderNumber)
			...
			<TR BGCOLOR="<?php echo $bgcolor ?>" VALIGN=TOP>
				...
				?></TD>
						...
						if ( $row['Status'] == "" or $row['Status'] == 'O') {
							...
							$Status="Approved";
							if ( $k == 0 and strtolower($row['PurchaseOrderType']) != "process" ) {
								$Status .= "<br />But should not";
								$color="red";
							}
							else {
								$color="#90EE90";
...
//if $pon == '' then add new PO, lookup database and get a temp PO# and continue the processes
  $PurchaseOrderNumber = (isset($_REQUEST['PurchaseOrderNumber'])) ? escape_data($_REQUEST['PurchaseOrderNumber']) : "";
  $show_add_prd = false;
  if ( $pon == '' ) {
	$sql = "SELECT MAX(PurchaseOrderNumber) FROM purchaseordermaster where PurchaseOrderNumber <> 820010";
	$result=mysql_query($sql,$link) or die (mysql_error() ." Failed execute SQL : $sql <br />");
	$row=mysql_fetch_array($result);
	$pon = $row[0] + 2;
	$now = date("Y-m-d H:m:s");
	$sql = "INSERT INTO purchaseordermaster (PurchaseOrderNUmber, DateOrderPlaced) values( $pon,'$now')";
	mysql_query($sql,$link) or die (mysql_error() . " Failed execute SQL : $sql <br />");
	$show_add_prd = true;
} else if ( $pon != $PurchaseOrderNumber ) { //use user provided PO
	$sql = "SELECT * FROM purchaseordermaster where PurchaseOrderNumber = '$PurchaseOrderNUmber'";
	$result = mysql_query($sql,$link) or die ( mysql_error() ." Failed Execute SQL : $sql <br />");
	$match_found = mysql_num_rows($result);
	if ($mact_found > 0 ) {
		$error_found = true;
		$error_message .= "P.O.# already exists<BR>";
	}
	...
	if ( ! error_found ) {
		...
		$sql="UPDATE purchaseordermaster set PurchaseOrderNumber = '$PurchaseOrderNumber'
		WHERE PurchaseOrderNumber='$pon'";
		mysql_query($sql,$link) or die (mysql_error() ."Failed execute SQL : $sql <br />");
		
		$sql="UPDATE purchaseorderdetail set PurchaseOrderNumber = '$PurchaseOrderNumber' 
		WHERE PurchaseOrderNumber='$pon'";
		mysql_query($sql,$link) or die (mysql_error() ."Failed to execute SQL : $sql <br />");
		
		$pon=$PurchaseOrderNumber;
		$sql="DELETE FROM purchaseordermaster WHERE PurchaseOrderNumber='$pon'";
		mysqy_query($sql,$link) or die (mysql_error() ."Failed to execute SQL : $sql <br />");
		
		$sql="DELETE FROM purchaseorderdetail WHERE PurchaseOrderNumber='$pon'";
		mysqy_query($sql,$link) or die (mysql_error() ."Failed to execute SQL : $sql <br />");
	}
...
?>
	...
	<div style="background-color:<?php echo ($status > 0) ? "yellow":"lawngreen" ?>;width:890px; padding:0.5em 0; margin-bottom:0.5em; font-size:2em ;font-weight:bold; text-align:center"><?php echo ($status > 0) ? "Open": "All Approved" ?>
			...
			<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="3">
				...
				<TR>
					...
					if ( !$error_found ) {
						...
						echo ( $PurchaseOrderNumber == "" ) ? $pon : $PurchaseOrderNumber;
					}
			...
			<TABLE BORDER="0" CELLPADDING="3" CELLSPACING="0">
				...
				<TR>
					...
					<td align="left"><select id="VendorLocation" <?php echo $form_status;?> style="width:190px"><option/><?php 
						...
						$sql = "SELECT DISTINCT vendor_addresses.address_id, vendor_addresses.*, vendors.vendor_id, 
						vendors.name,vendor_address_phones.number AS phone,vendor_address_phones.type 
						FROM vendors, vendor_addresses LEFT JOIN vendor_address_phones ON 
						(vendor_address_phones.address_id = vendor_addresses.address_id AND 
						vendor_address_phones.type = 2) WHERE vendor_addresses.vendor_id = vendors.vendor_id 
						AND vendors.active=1 AND vendor_addresses.active=1 AND vendors.name is not null " . $vendor_clause . 
						" ORDER BY name, country DESC, state, city";
						...
						while ( $row_vendors = mysql_fetch_array($result_vendors) ) {
							if ( $pre_vid != $row_vendors[vendor_id] )
				...
				<TR>
					...
					<TD><INPUT TYPE="text" ID="VendorName" NAME="VendorName" SIZE="20" VALUE="<?php echo stripslashes($VendorName);?>" <?php echo $form_status;?>>
					<INPUT TYPE="hidden" ID="VendorID" NAME="VendorID" VALUE="<?php echo $VendorID;?>"></TD>
				</TR>
				<TR>
		...
		<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0">
			<TR>
				...
				<TABLE ALIGN=RIGHT BORDER=0 CELLSPACING="0" CELLPADDING="2" BORDERCOLOR="#CDCDCD">
					...
					<TR>
						<TD><INPUT TYPE="text" NAME="PurchaseOrderSeqNumber" id="PurchaseOrderSeqNumber" VALUE="1" SIZE="5" STYLE="text-align:right"></TD>
...
<?php } ?>
	...
	$sql = "SELECT purchaseorderdetail.*, productmaster.Designation, productmaster.Natural_OR_Artificial, 
...
//	$sql = "SELECT * FROM purchaseorderdetail WHERE PurchaseOrderNumber = '" . $pon . "' ORDER BY PurchaseOrderSeqNumber";
	...
	if ( $c > 0 ) { ?>
		...
		while ( $row = mysql_fetch_array($result) ) {
			...
			<div name="add_vendid" id="add_vendid"></div>
				<TD>
				<?php if ( "" != $form_status and $_REQUEST['update_seq_no'] == '' ) { ?>
				...
				<?php if ( "" != $form_status ) { ?>
					...
					<?php //use editable dropdow or show hint? hint may be better.
							$sql = "SELECT VendorID, vendor_name VendorName, ProductNumberInternal,PriceEffectiveDate,PricePerPound,
							Volume,Packaging,Designation,Natural_OR_Artificial, Kosher, ProductPricesKosher, Notes, VendorProductCode
							FROM vwmaterialpricing
							WHERE ProductNumberInternal = " .$row['ProductNumberInternal'];
							//echo "<br />". $sql ."<br />";
							$sql_prdprice = mysql_query($sql,$link) or die (mysql_error()." Failed Execute SQL $sql <br />");
					} else if ( "" == $_REQUEST['update_seq_no'] ){ ?>
				...
				<TD><?php 
					...
					echo "[ ". $row['Description']. " ] ". $row['Natural_OR_Artificial'] . " " . 
					$kosher_info . $row['Designation'];
				...
				<?php if ( isset($sql_prdprice) ) { // print hint for po price and units
					echo "<TR><TD colspan='6'></TD><TD colspan='8' ALIGN='left'>";
					?>
					<TABLE>
						<TH bgcolor='white'>Vendor#</TH><TH>Internal#</TH><TH bgcolor='white'>Vendor</TH>
						<TH>Effect Date</TH><TH bgcolor='white'>Price/lb</TH>
						<TH>Package</TH><TH bgcolor='white'>Min.</TH><TH>Notes</TH>
						<TH>&nbsp;</TH></TR>
					<?php
					while ( $row_prdhint = mysql_fetch_array($sql_prdprice) ) {
						echo "<TR>\n";
						echo "<TD bgcolor='white'>". $row_prdhint['VendorProductCode'] ."</TD>\n";
						echo "<TD>".$row_prdhint['ProductNumberInternal'] ."</TD>\n";
						echo "<TD bgcolor='white'>" .$row_prdhint['VendorName'] ."</TD>\n";
						echo "<TD>" . substr($row_prdhint['PriceEffectiveDate'],0,10) ."</TD>\n";
						echo "<TD bgcolor='white'>". $row_prdhint['PricePerPound'] ."</TD>\n";
						echo "<TD>". ( empty($row_prdhint['Volume'] ) ? $row_prdhint['Packaging'] : $row_prdhint['Volume']) . "</TD>\n";
						echo "<TD bgcolor='white'>" . $row_prdhint['Minimums'] ."</TD>\n";
						echo "<TD>" . $row_prdhint['Kosher'] ." ". $row_prdhint['Notes'] ."</TD><TD>\n";
						if ( $VendorID == "" ) { ?>
							<INPUT TYPE='button' class='submit' name='select_vendor' VALUE='Select This Vendor' onClick="setVendorID('<?php echo $row_prdhint['VendorID'];?>')" >
						<?php }
						echo "</TD></TR>";
					}
					echo "</TABLE></TD></TR>";
...
function setVendorID(vid) {
	document.getElementById('add_vendid').innerHTML = "<INPUT TYPE='hidden' NAME='VendorID' id='VendorID' VALUE='"+vid+"'>";
}
// -->
</script>
